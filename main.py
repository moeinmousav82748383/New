...
OWNER_ID = 8007177524
DEVELOPER_ID = 8007177524
ADMIN_IDS = [OWNER_ID, DEVELOPER_ID]

DB_PATH = "selfsaz"
DIAMOND_RATE = 40
REF_BONUS = 40
BOT_USERNAME = "@Testttttrobotbot"
...
BALANCE_TEXT = (
    "💎 موجودی شما:\n"
    "الماس 💎: <code>{diamonds}</code>\n"
    "تومان : <code>{toman:,}</code>\n"
    "سکه 🪙: <code>0</code>\n"
    "به تومان : <code>0</code>"
)

FREE_DIAMOND_TEXT = (
    "💎 با دعوت دوستان خود\n"
    "50 الماس دریافت کنید! فقط تا امروز..\n"
    "👥 کل دعوتی‌ها: <code>{count}</code>\n"
    "🔗 لینک دعوت: <code>{link}</code>"
)
@bot.message_handler(func=lambda m: in_private(m) and m.text == "≼ الماس رایگان ≽")
def free_diamonds(m: types.Message):
    ...
    # فقط همین متن ارسال میشه — هیچ موجودی‌ای نیست
    bot.send_message(
        user_id,
        FREE_DIAMOND_TEXT.format(count=count, link=link),
        parse_mode="HTML"
    )

    # بنر
    banner_url = "https://i.postimg.cc/Prr7YBhG/IMG-20251114-171627-458.jpg"
    caption = (
        "🤖 ربات شرطبندی و سلف ساز (رایگان)\n\n"
        "🔰 تو این ربات میتونید شرطبندی کنید برنده شید و کلی چالش های خفن هر روز برگذار میشه میتونی سلف خفن رو اکانتت فعال کنی و کلی قابلیت های خفن بدو از دستش نده کاملا رایگان♻️\n\n"
        "👇🏻 همین الان وارد این ربات فوق العاده شو"
    )

    keyboard = types.InlineKeyboardMarkup()
    button = types.InlineKeyboardButton("✅ عضویت در ربات", url=link)
    keyboard.add(button)

    bot.send_photo(user_id, photo=banner_url, caption=caption, reply_markup=keyboard)
...
def cmd_start(m: types.Message):
    ...
    if in_private(m):
        # متن استارت جدید
        text = "⬮ به سلف 𝐕𝐢𝐏 خوش اومدی⬮"
        # لینک مستقیم عکس
        photo_id = "https://i.postimg.cc/HnrDGrnY/file-00000000dd08720ab18eee434cef0c48.png"
...
def bet_keyboard(bet_id: int, creator_id: int):
    ...
    kb.add(
        types.InlineKeyboardButton("✘ لغو شرط", callback_data=f"bet:cancel:{bet_id}:{creator_id}"),
        types.InlineKeyboardButton("◈ ــــ پیوستن ــــ ◈", callback_data=f"bet:join:{bet_id}")
    )
    return kb
...
def cmd_bet(m):
    ...
    msg = bot.send_photo(
        m.chat.id,
        photo="https://i.postimg.cc/7LWLJpJW/IMG-20251116-162852-664.jpg",
        caption=text,
        reply_markup=kb,
...
def cb_bet(c):
    try:
        ...
        return bot.answer_callback_query(c.id, "❌ داده نامعتبر.", show_alert=True)
    ...
    try:
        with db_lock:
            with sqlite3.connect(DB_PATH) as conn:
                cur = conn.cursor()
                cur.execute(
                    "SELECT creator_id, amount, state, player_joined_id, message_id FROM bets WHERE bet_id=?",
                    (bet_id,)
                )
                row = cur.fetchone()

        if not row:
            return bot.answer_callback_query(c.id, "❌ شرط پیدا نشد.", show_alert=True)
        ...
        if action == "cancel":
            if user_id != creator_id:
                return bot.answer_callback_query(c.id, "فقط سازنده شرط می‌تواند لغو کند.", show_alert=True)
            if state != "open":
                return bot.answer_callback_query(c.id, "❌ این شرط قبلاً بسته شده است.", show_alert=True)
            ...
            try:
                bot.edit_message_media(
                    media=types.InputMediaPhoto(
                        "https://i.postimg.cc/7LWLJpJW/IMG-20251116-162852-664.jpg",
                        caption="❌ این شرط توسط سازنده لغو شد."
                    ),
            ...
            return bot.answer_callback_query(c.id, "✅ شرط لغو شد.", show_alert=True)

        # --- JOIN ---
        if action == "join":
            if state != "open":
                return bot.answer_callback_query(c.id, "این شرط بسته شده است.", show_alert=True)
            if joined_id:
                return bot.answer_callback_query(c.id, "یک نفر قبلاً پیوسته است.", show_alert=True)
            if user_id == creator_id:
                return bot.answer_callback_query(c.id, "نمی‌توانید روی شرط خودتان شرکت کنید.", show_alert=True)
            if get_balance(user_id) < MIN_BET:
                return bot.answer_callback_query(c.id, " موجودی شما کافی نیست.", show_alert=True)

            change_balance(user_id, -MIN_BET)
            ...
            # نمایش در حال مشخص کردن برنده
            try:
                bot.edit_message_media(
                    media=types.InputMediaPhoto(
                        "https://i.postimg.cc/7LWLJpJW/IMG-20251116-162852-664.jpg",
                        caption="𝐕𝐈𝐏  𝐒𝐞𝐥𝐟 | در حال مشخص کردن برنده..."
                    ),
                    chat_id=c.message.chat.id,
                    message_id=message_id
                )
            ...
            with db_lock:
                with sqlite3.connect(DB_PATH) as conn:
                    cur = conn.cursor()
                    cur.execute(
                        "UPDATE bets SET state='closed', player_joined_id=? WHERE bet_id=?",
                        (user_id, bet_id)
                    )
                    conn.commit()
            ...
            # آخرین مرحله: هشدار پنجره باز شود
            return bot.answer_callback_query(c.id, "🎯 نتیجه شرطبندی", show_alert=True)

    except Exception as e:
        print("Bet Callback Error:", repr(e))
        try:
            bot.answer_callback_query(c.id, "❌ خطا رخ داد. دوباره تلاش کنید.", show_alert=True)
        except:
            pass
...
# ----------------- PRIVATE MENU -----------------
DIAMOND_RATE = 15  # قیمت هر الماس × 15 تومان
MIN_DIAMONDS = 1    # حداقل تعداد الماس

# ----------------- NUMPAD برای شارژ موجودی -----------------
def numpad():
    kb = types.InlineKeyboardMarkup(row_width=3)
    kb.add(
        types.InlineKeyboardButton("1", callback_data="num:1"),
        types.InlineKeyboardButton("2", callback_data="num:2"),
        types.InlineKeyboardButton("3", callback_data="num:3"),
        types.InlineKeyboardButton("4", callback_data="num:4"),
        types.InlineKeyboardButton("5", callback_data="num:5"),
        types.InlineKeyboardButton("6", callback_data="num:6"),
        types.InlineKeyboardButton("7", callback_data="num:7"),
        types.InlineKeyboardButton("8", callback_data="num:8"),
        types.InlineKeyboardButton("9", callback_data="num:9"),
        types.InlineKeyboardButton("⌫", callback_data="num:del"),
        types.InlineKeyboardButton("0", callback_data="num:0"),
        types.InlineKeyboardButton("✔️ تایید", callback_data="num:ok"),
    )
    kb.add(types.InlineKeyboardButton("❌ لغو", callback_data="num:cancel"))
    return kb

# ----------------- منوی شارژ -----------------
@bot.message_handler(func=lambda m: in_private(m) and m.text.strip() == "≼ شـارژ مـوجـودی 💳 ≽")
def charge_menu(m: types.Message):
    set_setting(f"charge_{m.from_user.id}", "")
    bot.send_message(
        m.chat.id,
        "🛒 خرید الماس\nتعداد الماس موردنظر را وارد کنید:\n—\n\nبا کیبورد زیر ادامه بده:",
        reply_markup=numpad()
    )

# ----------------- هندلر اعداد -----------------
@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("num:"))
def cb_numpad(c):
    action = c.data.split(":")[1]
    key = f"charge_{c.from_user.id}"
    current = get_setting(key) or ""
    ...
    if action == "del":
        ...
        if not current.isdigit() or int(current) < MIN_DIAMONDS:
            bot.answer_callback_query(c.id, f"❌ تعداد حداقل {MIN_DIAMONDS} باید باشد.", show_alert=True)
            return

        diamonds = int(current)
        toman = diamonds * DIAMOND_RATE

        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton("🛍 پشتیبانی خرید", url="https://t.me/im_horizen"))
        kb.add(types.InlineKeyboardButton("❌ لغو", callback_data="final_cancel"))

        bot.edit_message_text(
            f"🧾 **پیش‌فاکتور خرید الماس**\n\n"
            f"💎 تعداد: *{diamonds}*\n"
            f"💰 مبلغ قابل پرداخت: *{toman:,} تومان*\n\n"
            f"💳 شماره کارت: `000000000000000`\n"
            f"👤 بنام: *اسم فامیل*\n\n"
            f"✨ پس از پرداخت، رسید را برای پشتیبانی ارسال کن.\n"
            f"⚡ زیر ۱ دقیقه شارژ می‌شی.",
            c.message.chat.id,
            c.message.message_id,
            reply_markup=kb,
            parse_mode="Markdown"
    ...
    # اگر عددی وارد شده باشه، خط جایگزین عدد میشه (mono / تک ضرب)
    display = current if current else "—"

    bot.edit_message_text(
        f"🛒 خرید الماس\n\nهر الماس: {DIAMOND_RATE} تومان\nحداقل: {MIN_DIAMONDS}\nتعداد:\n{display}\n\nبا کیبورد زیر ادامه بده:",
        c.message.chat.id,
        c.message.message_id,
        reply_markup=numpad()
    )
...
# ----------------- دکمه سلف -----------------
@bot.message_handler(func=lambda m: in_private(m) and m.text.strip() == "≼ سـلـفـ 𝐕𝐢𝐏 ≽")
def self_vip(m: types.Message):
    bot.send_message(m.chat.id, "در حال آپدیت...")

# ----------------- دکمه خدمات -----------------
@bot.message_handler(func=lambda m: in_private(m) and m.text.strip() == "≼ خـدمـاتـ 𝐕𝐢𝐏 ≽")
def services_vip(m: types.Message):
    bot.send_message(
        m.chat.id,
        "🛠️ خدمات VIP:\n\n"
        "• شرطبندی در گروه‌ها\n"
