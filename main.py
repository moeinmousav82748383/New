...
def main_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔥 تشخیص ضریب", callback_data="detect_crash")]])

def next_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔥 تشخیص ضریب دست بعدی", callback_data="detect_crash")]])
...
async def detect_crash(callback):
    ...
    await callback.message.edit_text("🔍 <b>در حال تشخیص ضریب...</b>", parse_mode="HTML")
...
async def other_messages(message):
    await message.answer("لطفاً از دکمه‌های موجود استفاده کن Moein جان.", reply_markup=main_keyboard())
